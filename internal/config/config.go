...
type Config struct {
	Name                string   `default:"admission-webhook-k8s" desc:"Name of current admission webhook instance"`
	ServiceName         string   `default:"default" desc:"Name of service that related to this admission webhook instance"`
	Namespace           string   `default:"default" desc:"Namespace where admission webhook is deployed"`
	Annotation          string   `default:"networkservicemesh.io" desc:"Name of annotation that means that the resource can be handled by admission-webhook"`
	NSURLEnvName        string   `default:"NSM_NETWORK_SERVICES" desc:"Name of env that contains NSURL in initContainers/Containers"`
	InitContainerImages []string `desc:"List of init containers that should be appended for each deployment that has Config.Annotation"`
	ContainerImages     []string `desc:"List of containers that should be appended for each deployment that has Config.Annotation"`
	Envs                []string `desc:"Additional Envs that should be appended for each Config.ContainerImages and Config.InitContainerImages"`
	CertFilePath        string   `desc:"Path to certificate"`
	KeyFilePath         string   `desc:"Path to RSA/Ed25519 related to Config.CertFilePath"`
	CABundleFilePath    string   `desc:"Path to cabundle file related to Config.CertFilePath"`
	envs                []corev1.EnvVar
	caBundle            []byte
